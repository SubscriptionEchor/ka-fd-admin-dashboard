...
function Addon(props) {
  ...
  const onChange = (event, index, state) => {
    ...
    // For quantityMinimum (default 0)
    if (state === 'quantityMinimum') {
      // If current value is 0 and user is typing, replace the 0
      if ((addons[index][state] === 0 || addons[index][state] === '0') && value !== '') {
        addons[index][state] = value.replace(/^0+/, '');
      } else {
        addons[index][state] = value;
      }
    } 
    // For quantityMaximum (default 1)
    else if (state === 'quantityMaximum') {
      // If current value is 1 and user is typing, replace the 1
      if ((addons[index][state] === 1 || addons[index][state] === '1') && value !== '') {
        addons[index][state] = value.replace(/^1+/, '');
      } else {
        addons[index][state] = value;
      }
    }
    // For other inputs, keep existing behavior
    else {
      addons[index][state] = value;
    }
    ...
    <Box container className={[classes.container, classes.width60]}>
      ...
      <Box className={classes.form}>
        {addon.map((addonItem, index) => (
          <Box className={index !== 0 ? classes.nf : ''} key={index}>
            <Box className={classes.addRemoveButtons}>
