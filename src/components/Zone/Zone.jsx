...
const Zone = props => {
  ...
  const [center] = useState(
    props.zone
      ? setCenter(props.zone.location.coordinates[0])
      : { lat: 17.385044, lng: 78.486671 }  // Changed to Hyderabad coordinates
  )
  ...
  const clearBounds = () => {
    setPath([]); // Clear the polygon path
  };
  const { t } = props
        ...
        <form>
          ...
          <Box mt={2} className={globalClasses.flexRow}>
            {/* <GoogleMap
              ...
              id="example-map"
              zoom={6}
              center={props.zone ? center : { lat: 28.6139, lng: 77.2090 }} // Default to Delhi if no zone
              onLoad={map => {
                if (props.zone && path.length > 0) {
            ...
            <GoogleMap
              ...
              id="example-map"
              zoom={10}
              center={props.zone ? center : { lat: 17.385044, lng: 78.486671 }} // Default to Hyderabad
              onLoad={map => {
                if (props.zone && path.length > 0) {
                  const bounds = new window.google.maps.LatLngBounds()
                  path.forEach(point => bounds.extend(point))
